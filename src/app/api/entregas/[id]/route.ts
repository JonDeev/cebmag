...
export async function GET(_req: NextRequest, ctx: { params: any }) {
  ...
  const item = await prisma.entrega.findUnique({
    where: { id },
    include: { beneficiario: { select: { id: true, doc: true, nombres: true, apellidos: true, direccion: true } } },
  });
...
export async function PATCH(req: NextRequest, ctx: { params: any }) {
  try {
    ...
    if (typeof body.observaciones !== "undefined") patch.observaciones = body.observaciones ? upper(body.observaciones) : null;

    // dirección se guarda en adjuntos.meta.direccionEntrega
    if (typeof body.adjuntos !== "undefined" || typeof body.direccion !== "undefined") {
      // traemos adjuntos actuales para no perder meta si no llega
      const current = await prisma.entrega.findUnique({ where: { id }, select: { adjuntos: true } });
      const unpack = unpackAdjuntos(current?.adjuntos);

      const files = typeof body.adjuntos !== "undefined" ? body.adjuntos : unpack.files;
      const dir = typeof body.direccion !== "undefined" ? String(body.direccion ?? "").trim() : unpack.direccionEntrega;
